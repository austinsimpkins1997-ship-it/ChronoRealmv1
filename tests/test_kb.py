...
FONT = next((p for p in ["/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf", os.path.join(os.environ.get("WINDIR", "C:/Windows"), "Fonts", "arial.ttf")] if os.path.exists(p)), "")
try:
...
def write_text_pdf(path, pages):
    # Small real PDF fixture; avoids requiring a Python PDF library for this test.
    kids = " ".join(f"{4+2*i} 0 R" for i in range(len(pages)))
    objects = [b"<< /Type /Catalog /Pages 2 0 R >>",
               f"<< /Type /Pages /Kids [{kids}] /Count {len(pages)} >>".encode(),
               b"<< /Type /Font /Subtype /Type1 /BaseFont /Courier >>"]
    for i, text in enumerate(pages):
        lines = [text[j:j+80] for j in range(0, len(text), 80)]
        escaped = [x.replace(chr(92), chr(92)*2).replace("(", chr(92)+"(").replace(")", chr(92)+")") for x in lines]
        stream = ("BT /F1 9 Tf 30 760 Td 12 TL " + " ".join("("+x+") Tj T*" for x in escaped) + " ET").encode("ascii")
